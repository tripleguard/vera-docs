...
Вера, открой ютуб
```
Погода определяется по указанному городу. Если город в запросе не назван, используется поле профиля `город` из памяти пользователя. Профиль можно заполнить обычной командой памяти, например: «Вера, запомни мой город Москва». В ответе показываются текущая температура, погодные условия и короткая рекомендация по одежде или осадкам.
